    ...
    quickActionBtns.forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            const actions = {
                ...
                'Create Job': () => showToast('Create Job functionality would be implemented here', 'success'),
                'Update Status': () => showToast('Update Status functionality would be implemented here', 'success'),
                'View Reports': () => showToast('View Reports functionality would be implemented here', 'success')
            };
...
function initForms() {
    ...
    document.getElementById("pin").addEventListener("input", function() {
        ...
        validateField("pin", /^\d{6}$/.test(pin), "Enter a valid 6-digit pin code.");
    ...
    document.getElementById("productType").addEventListener("change", function() {
        validateField("productType", this.value !== "", "Please select a product type.");
    ...
    document.getElementById("purchaseDate").addEventListener("input", function() {
        ...
        validateField("purchaseDate", this.value !== "", "Purchase date is required.");
    });
    ...
    document.querySelectorAll('input[name="callType"]').forEach(radio => {
        radio.addEventListener("change", () => {
            ...
            callTypeError.textContent = document.querySelector('input[name="callType"]:checked') ? "" : "Please select a call type.";
        });
    });
    ...
    document.querySelectorAll('input[name="priority"]').forEach(radio => {
        radio.addEventListener("change", () => {
            ...
            priorityError.textContent = document.querySelector('input[name="priority"]:checked') ? "" : "Please select call priority.";
        });
    });
    ...
    callForm.addEventListener('submit', function (e) {
        ...
        if (!document.querySelector('input[name="callType"]:checked')) {
            document.getElementById("callTypeError").textContent = "Please select a call type.";
            isValid = false;
        }
        if (!document.querySelector('input[name="priority"]:checked')) {
            document.getElementById("priorityError").textContent = "Please select call priority.";
            isValid = false;
        }
...
function initUI() {
    ...
    document.querySelectorAll('.product-actions .btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            const action = this.textContent.includes('View Details') ? 'Viewing' : 'Creating job';
    ...
    document.querySelectorAll('.more-btn').forEach(btn => {
        btn.addEventListener('click', () => showToast('More options menu would appear here', 'success'));
    });

    const notificationBtn = document.querySelector('.notification-btn');
    if (notificationBtn) {
        notificationBtn.addEventListener('click', () => showToast('Notifications panel would open here', 'success'));
    }
...
function showToast(message, type = 'success', duration = 3000) {
    ...
    if (Array.from(container.children).some(toast => toast.textContent === message)) {
        return;
    }
