...
function initializeModals() {
    const modalIds = ['viewPoModal', 'editPOModal', 'editGrmModal'];
        ...
        if (modal) {
            ...
            if (closeBtn) {
                closeBtn.addEventListener('click', function() {
                    ...
                    else if (modalId === 'editPOModal') closeEditPoModal();
...
window.editPOOrder = function(orderData) {
  try {
    ...
    const modal = document.getElementById("editPOModal");
        if (!modal) {
            console.error("editPoModal element not found in DOM");
            showToast("Error: edit Po modal not found", "error");
            return;
        }
    ...
    // Show modal
        showModal(modal);
    console.log("Edit PO Modal should now be visible");
  } 
